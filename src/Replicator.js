...
class Replicator extends EventEmitter {
   ...
   */
  load (entries) {
    const notKnown = entry => !this._store._oplog.has(entry.cid || entry) && !this._queue[entry.cid || entry]
  ...
  async _processOne (entry) {
    ...
    const exclude = []
    const log = await Log.fromEntryCid(this._store._ipfs, this._store.access, this._store.identity, cid, this._store._oplog.id, batchSize, exclude)
